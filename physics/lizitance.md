...
![image-20250323123438340](./img/lizitance/image-20250323123438340.png)

> 第一个求导是因为积分中除了指数上有t其他地方没有
>
> 第二个呢，是把e指数泰勒展开，然后注意到x的n次方可以写成求导的形式



### 二项分布

一个事件总共进行n次（比如抛硬币），发生k次硬币向上的概率

![image-20250323124706888](./img/lizitance/image-20250323124706888.png)

显然，这个事件出现的平均次数，以及方差

![image-20250323124837466](./img/lizitance/image-20250323124837466.png)

放射衰变也可以使用二项分布，众所周知，放射衰变是个指数分布，那么在时间t的时候剩余的粒子数除以一开始的粒子数可以当作一个衰变概率

![image-20250323125947256](./img/lizitance/image-20250323125947256.png)



### 泊松分布

在二项分布中，若**n大p小np适中**，则二项分布近视为泊松分布

注意泊松分布的描述

> 在独立实验中，用$p_n$代表事件A出现的概率（A在一次实验中要么出现要么不出现，显然到目前为止，这个描述是二项的），**这个概率与试验次数n有关**，若$np_n \rightarrow \lambda$（注意这里没有描述n的大小，只说了np趋向于$\lambda$），在$n \rightarrow +\infin$时分布是泊松的

$$
b(k; n, p_n) \rightarrow p(k; \lambda) = \frac{e^{-\lambda} \lambda^k}{k!}
$$

性质（平均值和方差）

![image-20250323132102749](./img/lizitance/image-20250323132102749.png)

**应用**：

在核衰变中（上面我们使用了二项分布，这里我们用泊松）

n很大，p呢？在0-t时间内$p = (1 - e^{-\lambda t}) \approx \lambda t$，显然这个p不大

所以t时间内的总衰变数分布


$$
p(N; \lambda t N_0) = \frac{e^{-\lambda t N_0}(\lambda t N_0)^{N}}{N!}
$$


**定理：**若单位时间内的事件数服从泊松分布$p(n;\lambda) = \frac{\lambda^n e^\lambda}{n!}$，则两个事件之间的时间间隔t服从指数分布

$p(t) = \lambda e^{-\lambda t}$，这个时间的平均值为$\frac 1 \lambda$

**定性理解**：单位时间内（1时间），发生的事件数为$<n> = \lambda$，所以两两事件发生的时间间隔平均为$1 \over \lambda$



### 高斯分布

![image-20250323134442154](./img/lizitance/image-20250323134442154.png)

可以证明泊松分布在平均值很大的时候趋近于高斯

![image-20250323140456330](./img/lizitance/image-20250323140456330.png)



### 指数分布

![image-20250323141203300](./img/lizitance/image-20250323141203300.png)

这是个连续分布，所以f前面的估计是归一化系数



### 卡方分布

K个随机变量相互独立并且各自符合***标准*正态分布**则他们的和是卡方分布

![image-20250323165337862](./img/lizitance/image-20250323165337862.png)

![image-20250323165423391](./img/lizitance/image-20250323165423391.png)
$$
\begin{align}
	E[x] &= k
	\\
...
![image-20250323183200248](./img/lizitance/image-20250323183200248.png)

![image-20250323183212952](./img/lizitance/image-20250323183212952.png)

![image-20250323183226072](./img/lizitance/image-20250323183226072.png)



粒子计数通常符合泊松分布，当计数平均值很大的时候服从高斯

![image-20250323183845595](./img/lizitance/image-20250323183845595.png)

误差传播

![image-20250323183955909](./img/lizitance/image-20250323183955909.png)
